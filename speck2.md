...
11. **Gate-check the diff.** Ensure the speck diff only contains `*.speck` files with `git status --porcelain | grep -qv '\.speck' && exit 1 || exit 0` after every change to the diff. If any non-`.speck` file appears, you have leaked source changes: stop, stage or revert it, and investigate before continuing. The diff must contain speck files and nothing else.
